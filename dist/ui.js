 ...
 */
import { CombatEventType, Role } from './combat/types.js';
export class UIRenderer {
    ...
    updateEnemy(state) {
        ...
        if (dungeonState.isActive && dungeonState.currentDungeon) {
            if (dungeonInfoEl)
                dungeonInfoEl.style.display = 'block';
            if (dungeonNameEl)
                dungeonNameEl.textContent = `🏰 ${dungeonState.currentDungeon.name}`;
            ...
            if (dungeonInfoEl)
                dungeonInfoEl.style.display = 'none';
        }
    }
    /**
     ...
     */
    updateStatText(elementId, current, max) {
     ...
     */
    updateSkillButton(elementId, cooldownMs, state, skillNum) {
    ...
    updateInterruptButton(state) {
        const button = document.getElementById('interrupt-btn');
        if (!button)
            return;
     ...
     */
    updateDungeonEnemiesList(state) {
...
UIRenderer.MAX_LOG_ENTRIES = 10;
UIRenderer.ROLE_SKILL_NAMES = {

 ...
 */
import { StatusEffectType, calculateLevelInteraction } from './combat-types.js';
...
export class NewCombatEngine {
    ...
    processStatusEffects() {
        ...
        for (const combatant of allCombatants) {
            ...
            combatant.statusEffects = combatant.statusEffects.filter(e => !expiredEffects.includes(e.type));
    ...
    updateTelegraphs() {
        for (const telegraph of this.state.activeTelegraphs) {
            ...
            if (telegraph.remainingTicks === 1) {
                this.log('telegraph', `⚠️ ${this.getCombatant(telegraph.sourceId)?.name || 'Gegner'} führt Aktion im nächsten Tick aus!`, true);
            }
            if (telegraph.remainingTicks <= 0) {
                // Telegraph wurde ausgeführt, entfernen
                this.state.activeTelegraphs = this.state.activeTelegraphs.filter(t => t.actionId !== telegraph.actionId);
            }
        }
    }
    // ==================== PREPARED ACTIONS ====================
    ...
    // ==================== ACTION EXECUTION ====================
    executeAction(combatant, preparedAction) {
        ...
        if (action.baseDamage) {
            ...
            if (target.currentAction) {
                const tagResult = TagMatcher.checkInteraction(action.tags, target.currentAction.action.tags);
    ...
    // ==================== DAMAGE & HEALING ====================
    dealDamage(target, rawDamage, source) {
        ...
        // HP reduzieren
        target.hp -= damage;
        this.log('damage', `${target.name} erleidet ${Math.round(damage)} Schaden`, damage > 100);
    ...
    applyNewStatusEffect(target, statusApp, sourceLevel, interaction) {
    ...
    // ==================== PLAYER ACTIONS ====================
    playerUseAction(actionId, targetId) {
        ...
        const preparedAction = {
            ...
            targetId,
            canBeInterrupted: action.tags.some(t => t === 'kanalisierung' || t === 'ritual')
        };
        ...
        if (action.preparationTicks === 0) {
            ...
            this.log('telegraph', TelegraphSystem.formatTelegraph(telegraph, this.state.player.name), true);
        }
        // Mana abziehen
        if (action.manaCost) {
    ...
    processAI() {
        for (const enemy of this.state.enemies) {
            if (!enemy.isAI || enemy.currentAction || enemy.hp <= 0)
                continue;
    ...
    makeAIDecision(enemy) {
        ...
        if (action.preparationTicks === 0) {
            ...
            this.state.activeTelegraphs.push(telegraph);
            this.log('telegraph', TelegraphSystem.formatTelegraph(telegraph, enemy.name), true);
            // Bedrohungs-Warnung
            if (TelegraphSystem.isCritical(telegraph)) {
                this.log('system', TelegraphSystem.formatThreatWarning(telegraph.threatLevel), true);
            }
        }
    }
    // ==================== VICTORY CONDITIONS ====================
...
//# sourceMappingURL=new-engine.js.map
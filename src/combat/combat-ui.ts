...
import { NewCombatState, Combatant, Telegraph, CombatAction } from './combat-types.js';
import { TelegraphSystem } from './telegraph.js';
import { TagMatcher } from './tag-matcher.js';

export class CombatUI {
    private engine: NewCombatEngine;
    private container: HTMLElement;
    
    constructor(engine: NewCombatEngine, containerId: string) {
        this.engine = engine;
        const elem = document.getElementById(containerId);
        if (!elem) throw new Error(`Container ${containerId} not found`);
        this.container = elem;
    ...
    private setupEventListeners(): void {
        ...
        this.engine.on('combat-end', (data: any) => this.showVictoryScreen(data.victor));
    }
    ...
    private renderTelegraphs(state: NewCombatState): string {
    ...
    private renderEnemyHPBar(enemy: Combatant): string {
    ...
    private renderPlayerHPBar(player: Combatant): string {
    ...
    private renderPlayerSprite(player: Combatant): string {
    ...
    private renderSquadSprite(member: Combatant): string {
    ...
    private renderEnemySprite(enemy: Combatant): string {
    ...
    private renderStatusEffects(combatant: Combatant): string {
    ...
    private getActionIcon(action: CombatAction): string {
        // Icon basierend auf Action-Name oder Tags
        const name = action.name.toLowerCase();
        
        if (name.includes('schlag') || name.includes('hieb')) return '👊';
        if (name.includes('stich') || name.includes('durchbohren')) return '🗡️';
        if (name.includes('feuer') || name.includes('flammen')) return '🔥';
        if (name.includes('blitz') || name.includes('donner')) return '⚡';
        if (name.includes('eis') || name.includes('frost')) return '❄️';
        if (name.includes('schatten') || name.includes('dunkel')) return '🌑';
        if (name.includes('licht') || name.includes('heilig')) return '✨';
        if (name.includes('gift') || name.includes('vergift')) return '☠️';
        if (name.includes('schild') || name.includes('block')) return '🛡️';
        if (name.includes('heil') || name.includes('regen')) return '💚';
        if (name.includes('buff') || name.includes('stärk')) return '💪';
        if (name.includes('debuff') || name.includes('schwäch')) return '⬇️';
        if (name.includes('betäub') || name.includes('stun')) return '💫';
        if (name.includes('kritisch') || name.includes('wut')) return '💥';
        
        // Tag-basiert
        if (action.tags.includes('magie' as any)) return '🔮';
        if (action.tags.includes('schild' as any)) return '🛡️';
        if (action.tags.includes('schatten' as any)) return '🌑';
        
        return '⚔️'; // Default
    }
    
    private getActionType(action: CombatAction): string {
        ...
        if (action.tags.includes('schild' as any) || action.tags.includes('block' as any) || name.includes('block')) {
            return 'verteidigung';
        }
    ...
    private renderSkillBar(state: NewCombatState): string {
            ...
            <div class="skill-bar">
                ...
                <div class="skill-icons">
                    ${state.player.availableActions.map(action => {
                        ...
                        if (action.description) tooltipText += '\\n' + action.description;
                        if (action.manaCost) tooltipText += '\\n💧 ' + action.manaCost;
                        if (isOnCooldown) tooltipText += '\\n⏱️ ' + cooldown + ' Ticks';
    ...
    private renderCombatLog(state: NewCombatState): string {
    ...
    private attachActionHandlers(state: NewCombatState): void {
    ...
    private getCombatantById(state: NewCombatState, id: string): Combatant | undefined {
    ...
    private showVictoryScreen(victor: 'player' | 'enemy'): void {
        const state = this.engine.getState();

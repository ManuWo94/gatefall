...
import { ActionTag, TagInteraction, TAG_INTERACTIONS } from './combat-types.js';
...
export class TagMatcher {
    ...
    static checkInteraction(attackTags: ActionTag[], defenseTags: ActionTag[]): {
        ...
        for (const atkTag of attackTags) {
            for (const defTag of defenseTags) {
                const interaction = TAG_INTERACTIONS.find(
                    i => i.attackTag === atkTag && i.defenseTag === defTag
                );
                ...
                const reverseInteraction = TAG_INTERACTIONS.find(
                    i => i.defenseTag === atkTag && i.attackTag === defTag
                );
     ...
     */
    static canInterrupt(attackTags: ActionTag[], defenseAction: { tags: ActionTag[] }): boolean {
    ...
    static describeAction(tags: ActionTag[]): string {
        const descriptions: Record<ActionTag, string> = {

...
import { Telegraph, TelegraphType, ThreatLevel, CombatAction } from './combat-types.js';

export class TelegraphSystem {
    /**
     * Erstellt einen Telegraph für eine vorbereitete Aktion
     */
    static createTelegraph(
        action: CombatAction,
        sourceId: string,
        targetId: string
    ): Telegraph {
    ...
    static formatTelegraph(telegraph: Telegraph, sourceName: string): string {
        const typeMessages: Record<TelegraphType, string> = {
    ...
    static formatThreatWarning(threatLevel: ThreatLevel): string {
        const warnings: Record<ThreatLevel, string> = {

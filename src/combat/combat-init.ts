...
export class CombatSystem {
    private engine: NewCombatEngine | null = null;
    ...
    startCombat(options: {
        ...
        enemies: { name: string; level: number; isBoss?: boolean }[];
        squadMembers?: { name: string; level: number; role: CombatRole }[];
    }): void {
        // Spieler erstellen
        ...
        const enemies: Combatant[] = options.enemies.slice(0, 3).map((e, idx) => {
        ...
        const squadMembers: Combatant[] = (options.squadMembers || []).slice(0, 2).map((s, idx) => ({
            id: `squad_${idx}`,
            name: s.name,
            isPlayer: true,
            level: s.level,
        ...
        // Combat State
        const initialState: NewCombatState = {
            tickCount: 0,
...
export function startTestCombat(): void {
